...
using CloudSOA.ServiceHost.CoreWcf.Protos;
using Grpc.Core;
...
public class ComputeGrpcService : ComputeService.ComputeServiceBase
{
    ...
    public override async Task<ExecuteResponse> Execute(ExecuteRequest request, ServerCallContext context)
    {
        ...
        try
        {
            var result = await _service.ExecuteAsync(
                request.Action, request.Payload.ToByteArray(), context.CancellationToken);
        ...
        {
            _logger.LogError(ex, "Error executing '{Action}'", request.Action);
            return new ExecuteResponse
            {
                MessageId = Guid.NewGuid().ToString("N"),
                RequestMessageId = request.MessageId,
    ...
    public override Task<ServiceInfoResponse> GetServiceInfo(ServiceInfoRequest request, ServerCallContext context)
    {
        var resp = new ServiceInfoResponse { ServiceName = _service.ServiceName, Version = "1.0.0" };
